...
Double_t lumibins[] = {0, 1.5, 3.0, 4.5, 6.0, 7.5, 10. };
  ...
  for (Int_t iset=0; iset < NSET; ++iset){
    for (Int_t ilumi=0; ilumi < NLUM+1; ++ilumi){
      sprintf(name,"eff1dpt_%1.0f_%1.0f_%1.0f",iset, 2,ilumi);
      heff1dpt[iset][0][ilumi] = (TH1D*)mcfile->Get(name);
      sprintf(name,"eff1dpt_%2.0f_%2.0f",iset,ilumi);
      heff1dpt[iset][1][ilumi] = (TH1D*)datafile2->Get(name);
      std::cout << name << std::endl;
      heff1dpt[iset][2][ilumi] = (TH1D*)(datafile2->Get(name)->Clone());
  ...
  for (Int_t ilumi=0; ilumi < NLUM+1; ++ilumi){
    for (Int_t iset=0; iset < NSET; ++iset){
      ...
      if (ilumi==NLUM){
	sprintf(outname,"ZSETEffCorrDataRatioParameters2Bin_%1d: ",iset);
      }
      else{
	sprintf(outname,"ZSETEffCorrDataRatioParameters2Bin%1d_%1d: ", ilumi,iset);
      }
