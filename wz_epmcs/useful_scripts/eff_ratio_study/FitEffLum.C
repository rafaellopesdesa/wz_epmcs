...
void FitEff::Loop(const bool BKG, const int MC)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   //const bool BKG= kTRUE;
   //const bool MC = kFALSE;
   if (MC > 0)
     assert(BKG==kFALSE);
   

   Double_t trk_prob_cut = -0.5;
   ...
   if (MC==1)
     fileout = new TFile("LumStudy_BigLumi_MC.root","RECREATE");
   else if (MC==2)
     fileout = new TFile("LumStudy_BigLumi_fast.root","RECREATE");
   else {
     if (BKG)
       fileout = new TFile("LumStudy_BigLumi_Bkg.root","RECREATE");
     else
       fileout = new TFile("LumStudy_BigLumi_Data.root","RECREATE");
   }
   
   Double_t hmX_cut = 12.;
   Double_t hmX_Invertcut = 30.;
   TH1D *ept[NSET][4];
   ...
   for (Int_t iset=0; iset < NSET; ++iset){
	 ...
	 for (Int_t ilumi =0; ilumi < NLUM+1; ++ilumi){
	   if (ilumi==NLUM){
	     ...
	     sprintf(name,"zMass_%i_%i_%i_%i",iset,ipt,ihist, ilumi);
	     sprintf(title,"Z Mass %2.0f < SET < %2.0f, %2.0f < pT < %2.0f, %i trk, %2.0f < lumi < %2.0f ", setbins[iset],setbins[iset+1],ptbins[ipt],ptbins[ipt+1],ihist, lumibins[ilumi], lumibins[ilumi+1]); 
	   }
	   zMass[iset][ipt][ihist][ilumi] = new TH1D(name,title, 75,59.,121.);
	 }
       }
     }
   }
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     bool useE1 = kFALSE;
     bool useE2 = kFALSE;
     bool useE1_op = kFALSE;
     bool useE2_op = kFALSE;

     useE1 =  (em_hmX[0] < hmX_cut);
     useE2 =  (em_hmX[1] < hmX_cut);
     
     if (BKG){
       useE1_op =  (em_hmX[0] > hmX_Invertcut) ;
       useE2_op =  (em_hmX[1] > hmX_Invertcut);
     }
     
     if ((!BKG) && !(useE1 && useE2) && (!(MC==2))){
       continue;
     }
     if ((BKG) && !(useE2_op || useE1_op)) {
       continue;
     }
     if ((MC==2) && (useE1 && useE2)){
       continue;
     ...
     while(em_pt[0] > ptbins[ptBine1+1]){
       ++ptBine1;
       if (ptBine1==(NPT-1))
	 break;
     }
     while(em_pt[1] > ptbins[ptBine2+1]){
       ++ptBine2;
       if (ptBine2==(NPT-1))
	 break;
     }
     //  std::cout << ScalarEtBin << "   " << ptBine1 << "   " << ptBine2 << "   " << LumiBin << std::endl;	
     Float_t weight = 1.;
     zMass[ScalarEtBin][ptBine1][0][LumiBin]->Fill(Mz,weight);
     zMass[ScalarEtBin][ptBine2][0][LumiBin]->Fill(Mz,weight);
     zMass[ScalarEtBin][ptBine1][0][NLUM]->Fill(Mz,weight);
     zMass[ScalarEtBin][ptBine2][0][NLUM]->Fill(Mz,weight);
     ept[ScalarEtBin][0]->Fill(em_pt[0],weight);
     ept[ScalarEtBin][0]->Fill(em_pt[1],weight);
     
     if ((em_trk_prob[0] > trk_prob_cut) || ( em_trk_prob[1] > trk_prob_cut)){
       for (Int_t iem=0; iem < 2; ++iem){
       if (em_trk_prob[iem] > trk_prob_cut) {
	   if (iem==0){
	     ept[ScalarEtBin][1]->Fill(em_pt[0],weight);
	     zMass[ScalarEtBin][ptBine1][1][LumiBin]->Fill(Mz,weight);
	     zMass[ScalarEtBin][ptBine1][1][NLUM]->Fill(Mz,weight);
	     ept[ScalarEtBin][2]->Fill(em_pt[1],weight);
	     zMass[ScalarEtBin][ptBine2][2][LumiBin]->Fill(Mz,weight);
	     zMass[ScalarEtBin][ptBine2][2][NLUM]->Fill(Mz,weight);
	     if (em_trk_prob[1] > trk_prob_cut) {
	       ept[ScalarEtBin][3]->Fill(em_pt[1],weight);
	       zMass[ScalarEtBin][ptBine2][3][LumiBin]->Fill(Mz,weight);
	       zMass[ScalarEtBin][ptBine2][3][NLUM]->Fill(Mz,weight);
