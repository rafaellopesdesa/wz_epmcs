...
void run_trig_ccec(){
  ...
  TPostScript *myps = new TPostScript("trig_eff_alt_withbkg_ccec_v12_11.ps",111);
  // mass range
  ...
  //TH2D *before = (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_All_CCEC");
  //TH2D *after = (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_Matched_CCEC");

  TH2D *before = (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_All_Triglist_CCEC_2");
  TH2D *after = (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_Matched_CCEC_2_0");
  
  
  before->Add( (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_All_Triglist_CCEC_1") ) ;
  after->Add( (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_Matched_CCEC_1_0") );

  before->Add( (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_All_Triglist_CCEC_0") );
  after->Add( (TH2D *)gROOT->FindObject("Pt_InvMass_EM_Probe_Trig_Matched_CCEC_0_0") );
  ...
  TH2D *sig2 = (TH2D *)gROOT->FindObject("smeared_hZcandMass_V_ElecPt_CCEC");
  ...
  for (int xbin = 0; xbin < NBins_X; xbin ++){
    ...
    if (bkgnd_mass[xbin]->Integral()>30){
      ...
      func -> SetParameters(1.40381e+02 , 7.39945e+01 ,9.41466e+00 ); 
  ...
  cout<<"double ccec_mc_entry_after["<<NBins<<"]={";
  for(int i=0; i<NBins; i++) {
    cout<<mc_entry_after[i]<<",";
  }
  cout<<"}"<<endl;
  cout<<"double ccec_mc_error_after["<<NBins<<"]={";
  for(int i=0; i<NBins; i++) {
    cout<<mc_error_after[i]<<",";
  }
  cout<<"}"<<endl;
  cout<<"double ccec_mc_entry_before["<<NBins<<"]={";
  for(int i=0; i<NBins; i++) {
    cout<<mc_entry_before[i]<<",";
  }
  cout<<"}"<<endl;
  cout<<"double ccec_mc_error_before["<<NBins<<"]={";
  for(int i=0; i<NBins; i++) {
    cout<<mc_error_before[i]<<",";
  }
  cout<<"}"<<endl;
  cout<<"double ccec_variable[NBins]={";
  for(int i=0; i<NBins; i++) {
    cout<<variable[i]<<",";
  }
  cout<<"}"<<endl;
  ...
  TF1 *efffit2 = new TF1("efffit2","0.5 * [2] * (1. + TMath::Erf((x-[0])/(sqrt(2)*[1])))*(1. + TMath::Erf((x-[4])/(sqrt(2)*[3])))",15,45 );
