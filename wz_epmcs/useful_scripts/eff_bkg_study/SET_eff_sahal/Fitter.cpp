...
void Fitter::Update(TH1 *hdata, TH1 *hsig, TH1 *hbkg, double xmin, double xmax, double step) {
  ...
  _step = step;
  _W1min= 0.1*(_hdata->Integral()/_hsig->Integral());
  _W2min= 0.0*(_hdata->Integral()/_hbkg->Integral());
  _W1max= 1.1*(_hdata->Integral()/_hsig->Integral());
  _W2max= 0.6*(_hdata->Integral()/_hbkg->Integral()); 
  _W1 = 0.9*(_hdata->Integral()/_hsig->Integral());  // _W1min + _step;
  std::cout << "SYY " << _hdata->Integral() << "   " << _W1*_hsig->Integral() << "   " << _hbkg->Integral() << std::endl;
...
void Fitter::GetDataSignalBkg(double &data, double &mc, double &bkg, double &mcerr, double &chisq) {
  ...
  if (_W1>0.0000000001){
    mcerr = _W1err*mc/_W1;
    //mcerr = sqrt(data + data*data*_W2err*_W2err + _W2*bkg );
    //mcerr = _W2err*bkg/_W2;
  } else {
...
Double_t Fitter::MinuitFit() {
  ...
  std::cout << "SY : " <<  _W1 << "   " << _W1min << "   " << _W1max << std::endl;	
 std::cout << "SY : " <<  _W2 << "   " << _W2min << "   " << _W2max << std::end
l;
  ...
  //{
      ...
      cout<<"Parameter fit  "<<0<<" value "<<_W1<<" err "<<_W1err<< "    " << _W1min << endl;
