...
for (Int_t iset=0; iset < NSET; ++iset){
  ...
  for (int ilumi=0; ilumi < NLUM+1; ++ilumi){
    for (int iset=0; iset < NSET; ++iset){
      ...
      if(zMassdatadiff2_tmp[iset][ipt][ilumi]->Integral()>0. && zMasssigdiff2_tmp[iset][ipt][ilumi]->Integral()>0. && zMassbkgdiff2_tmp[iset][ipt][ilumi]->Integral()>0.) {
	fitter.Update(zMassdatadiff2_tmp[iset][ipt][ilumi],zMasssigdiff2_tmp[iset][ipt][ilumi] ,zMassbkgdiff2_tmp[iset][ipt][ilumi], xmin, xmax);
	fitter.FitMass();
	...
	fitter.GetDataSignalBkg(data_entry_before_all[iset][ipt][ilumi], mc_entry_before_all[iset][ipt][ilumi], bkg_entry_before_all[iset][ipt][ilumi], mc_error_before_all[iset][ipt][ilumi], chisq1_all[iset][ipt][ilumi]);
      ...
      if(zMassdata_tmp[iset][ipt][3][ilumi]->Integral()>0. && zMasssig_tmp[iset][ipt][3][ilumi]->Integral()>0. && zMassbkg_tmp[iset][ipt][3][ilumi]->Integral()>0.) {
	fitter.Update(zMassdata_tmp[iset][ipt][3][ilumi],zMasssig_tmp[iset][ipt][3][ilumi] ,zMassbkg_tmp[iset][ipt][3][ilumi] , xmin, xmax);
	fitter.FitMass();
	fitter.Compare(ctemp);   
...
/////end all-events
  ...
  TFile *file1 = new TFile("eff_plots2.root", "recreate");
  TString name_plot;
  name_plot+="eff_data";
