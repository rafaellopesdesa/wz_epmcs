...
int main(int argc, char** argv) {
  ...
  // input file name
  std::string inputfile = steer.getString("inputfile");
  // output file name
  ...
  std::cout << "DepVarName = " << DepVarName1 << std::endl;
    ...
    for (int j=0; j<Nbins1; j++){
      for (int k=0; k<Nbins2; k++){
        for (int l=0; l<Nbins3; l++){
          ...
          char str_depvar1[50], str_depvar2[50], str_depvar3[50]; 

          // dimension 1
          if (ElectronDepVar1) sprintf(str_elecdep1, "[0]");
          else sprintf(str_elecdep1, "");
          sprintf(str_depvar1, "%s%s>%e&&%s%s<%e", DepVarName1.c_str(), str_elecdep1, DepVarBins1.at(j), DepVarName1.c_str(), str_elecdep1, DepVarBins1.at(j+1));

          // dimension 2
          if (DepVarDimension>1) {
            if (ElectronDepVar2) sprintf(str_elecdep2, "[0]");
            else sprintf(str_elecdep2, "");
            sprintf(str_depvar2, "%s%s>%e&&%s%s<%e", DepVarName2.c_str(), str_elecdep2, DepVarBins2.at(k), DepVarName2.c_str(), str_elecdep2, DepVarBins2.at(k+1));
          }
          else {
          ...
          if (DepVarDimension>2) {
            ...
            sprintf(str_depvar3, "%s%s>%e&&%s%s<%e", DepVarName3.c_str(), str_elecdep3, DepVarBins3.at(l), DepVarName3.c_str(), str_elecdep3, DepVarBins3.at(l+1));
          }
          else {
