...

{
  ...
  TFile* flos = new TFile("rootfiles/effratio_npnm_loose_pt_effrange_nout_ec_tg.root");  
  TFile* ftig = new TFile("rootfiles/effratio_npnm_tight_pt_effrange_nout_ec_tg.root");  
  TFile* fout = new TFile("rootfiles/compare_npnm_tight_loose_pt_effrange_nout_ec_tg.root", "recreate");
  ...
  hall->GetXaxis()->SetTitle("ElecPt (GeV)");
  ...
  TF1* fcall = new TF1("fcall", "pol3", 25, 75);
  fcall->SetLineColor(kBlue);

  hall->Fit(fcall, "R", "", 25, 55);
  ...
  std::cout << "SETEffDataHackPtDepParameters: " ;
  for (int i=0; i<=3; i++){
