...
int MakePlotsRaf2(TString file="Znunu_Znunurun_PostPMCS_Converted.root", double minlumi = -1., double maxlumi= 9999., double minupt = -1, double maxupt= 9999.) {
...
// data.SetBranchAddress("ZBeventNum",&ZBeventNum);
  ...
  for (Long64_t ientry=0; ientry <= data.GetEntries(); ientry++) {
    ...
    //   if(u_vec.DeltaPhi(utrue_vec2) > 0) chargeratio_uphi[zptbin]->Fill(TVector2::Phi_0_2pi(u_vec.Phi()), 1);
    // if(u_vec.DeltaPhi(utrue_vec2) < 0) chargeratio_uphim[zptbin]->Fill(TVector2::Phi_0_2pi(u_vec.Phi()), 1);

    //if(u_vec.DeltaPhi(utrue_vec2) > 0) chargeratio_uphi[0]->Fill(TVector2::Phi_0_2pi(u_vec.Phi()), 1);
    //if(u_vec.DeltaPhi(utrue_vec2) < 0) chargeratio_uphim[0]->Fill(TVector2::Phi_0_2pi(u_vec.Phi()), 1);

    //if(u_vec.DeltaPhi(utrue_vec2) > 0) chargeratio_uphi[zptbin]->Fill(delta_phi, 1);
    //if(u_vec.DeltaPhi(utrue_vec2) < 0) chargeratio_uphim[zptbin]->Fill(delta_phi, 1);
    //
    //if(u_vec.DeltaPhi(utrue_vec2) > 0) chargeratio_uphi[0]->Fill(delta_phi, 1);
    //if(u_vec.DeltaPhi(utrue_vec2) < 0) chargeratio_uphim[0]->Fill(delta_phi, 1);
  ...
  TFile* output = new TFile("plotsINPUTNEW.root", "RECREATE");
...
chargeratio_uphi[0]->Divide(chargeratio_uphim[0]);
 ...
 set_lumi_hist[0]->Write();
  ...
  for (Int_t i=0; i<zptbins+1; i++) {  
    //chargeratio_uphi[i]->Divide(chargeratio_uphim[i]);
    //chargeratio_uphi[i]->Write();
    //chargeratio_uphim[i]->Write();
